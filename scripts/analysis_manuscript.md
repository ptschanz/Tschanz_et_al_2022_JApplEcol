...
sampling rounds, within fields per
![400 m^2](https://latex.codecogs.com/png.image?%5Cdpi%7B110%7D&space;%5Cbg_white&space;400%20m%5E2 "400 m^2")
sampling area, i.e., the number of nests counted within the eight
![50\times 1m](https://latex.codecogs.com/png.image?%5Cdpi%7B110%7D&space;%5Cbg_white&space;50%5Ctimes%201m "50\times 1m")
belt transects:
...
Number of individuals per species captured within the
![400m^2](https://latex.codecogs.com/png.image?%5Cdpi%7B110%7D&space;%5Cbg_white&space;400m%5E2 "400m^2")
sampling area (CT = conventionally tilled, NT = no-till) of the 25
...
``` r
ci <- confint(modlist$tilsys_abu$model)
```

    ## Waiting for profiling to be done...

``` r
# get untransformed estimates and ci's
est_ct <- coef(summary(modlist$tilsys_abu$model))[1,"Estimate"] #(Intercept)
est_nt <- coef(summary(modlist$tilsys_abu$model))[2,"Estimate"] #mngt_soilNT
ci_ct <- ci[1,]
ci_nt <- ci[2,]
...
newdat$fit <- c(exp(est_ct), exp(est_ct + est_nt))
newdat$lwr <- c(exp(ci_ct)[1], exp(est_ct + ci_nt)[1])
newdat$upr <- c(exp(ci_ct)[2], exp(est_ct + ci_nt)[2])
newdat
```

    ##   mngt_soil      fit      lwr      upr
    ## 1   Tillage 5.166667 2.615993 11.79124
    ## 2   No-till 3.000000 1.027579  8.64768
